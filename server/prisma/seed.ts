...
async function main() {
    ...
    const employeeId = 'GOV001';
    const password = 'admin'; // In production, this should be complex

    const existingAdmin = await prisma.governmentUser.findUnique({
        where: { employee_id: employeeId }
    });

    if (!existingAdmin) {
        const passwordHash = await bcrypt.hash(password, 10);
        await prisma.governmentUser.create({
            data: {
        ...
        console.log(`✅ Seeded Government Admin: ${employeeId} / ${password}`);
    } else {
    ...
    console.log("✅ Seeded 5 Courses and 5 Scholarships");
}
